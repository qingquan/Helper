...
public class FacilitiesAdapter extends BaseAdapter {
	...
	public FacilitiesAdapter(Context context) {
		...
		for ( int i = 0; i < facilityNameStrings.length; i++ ) {
			FacilityInfo facilityInfo = new FacilityInfo(facilityNameStrings[i], i);
			facilityInfo.mIsClicked = PreferenceUtils.getBooleanValue(mContext, PreferenceUtils.KEY_FACILITY+i);
			mFacilities.add(facilityInfo);
		}
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if ( convertView == null )
			relativeLayout = (RelativeLayout) LayoutInflater.from(mContext).inflate(R.layout.facility_item, null);
		else 
	...
	public void setClicked(int position) {
		...
		PreferenceUtils.saveBooleanValue(mContext, PreferenceUtils.KEY_FACILITY+position, mFacilities.get(position).mIsClicked);
	}
